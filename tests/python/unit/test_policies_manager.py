...
def test_manager_update_only_records_trace_without_scoring():
    manager, tracer = _make_manager(policy="baseline_trace_similarity")
    seq_id = tracer.create_entry()

    manager.update_only(seq_id, np.array([[1, 2], [2, 3]]), layer_idx=0)

    entry = tracer.get_entry(seq_id)
    assert entry.matrix[0, 1] > 0
    assert entry.matrix[0, 2] > 0


def test_prefetch_enabled_respects_config_flag():
    manager, _ = _make_manager(policy="baseline_trace_similarity")
    assert manager.prefetch_enabled() is False

    manager, _ = _make_manager(policy="baseline_trace_similarity", topk=0)
    manager.config.prefetch = True
    assert manager.prefetch_enabled() is True


def test_history_reuse_library_stats_expose_queries_and_admissions():
    manager, tracer = _make_manager(policy="finegrained_history_reuse")
    seq_a = tracer.create_entry()
    tracer.update_entry(seq_a, np.array([[0, 1], [1, 0]]), layer_idx=0)
    manager.finish_sequence(seq_a)
    ...
    assert matrix is not None
    stats = manager.library_stats()
    assert stats["size"] == 1
    assert stats["admit_count"] == 1
