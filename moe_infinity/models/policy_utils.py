...
def drive_expert_policy(module, expert_index) -> None:
    ...
    if expert_policy is None or expert_prefetcher is None or seq_id_list is None:
        return
    if getattr(expert_prefetcher, "prefetch_policy_disabled", False) is True:
        return
    ...
    for batch_idx in range(batch_size):
        ...
        credit = None
        credit_enabled = (
            getattr(expert_prefetcher, "prefetch_credit_gated_enabled", False) is True
        )
        if credit_enabled and hasattr(expert_prefetcher, "speculation_credit"):
            ...
            if credit <= 0:
                ...
                if zero_action == "skip_policy_update":
                    if hasattr(expert_prefetcher, "record_credit_skip_policy_update"):
                        expert_prefetcher.record_credit_skip_policy_update()
                else:
                    expert_policy.update_only(
                        seq_id,
                        expert_index[batch_idx],
