...
class MoE:
    ...
    def _configure_hook(self, input_ids: torch.LongTensor):
        ...
        recorder = getattr(self.engine, "phasea_recorder", None)
        if recorder is not None:
            context = getattr(self.engine, "phasea_request_context", None) or {}
            if context:
                recorder.set_request_context(
                    request_id=str(context.get("request_id", "")),
                    trace_name=str(context.get("trace_name", "")),
                    tag=str(context.get("tag", "")),
                    variant=str(context.get("variant", "")),
                    is_warmup=bool(context.get("is_warmup", False)),
                )
            recorder.record_sequence_start(self.seq_id_list)
    ...
    def generate(self, input_ids: torch.LongTensor, **kwargs) -> Any:
        ...
        try:
            ...
            self.engine.phasea_request_context = None
